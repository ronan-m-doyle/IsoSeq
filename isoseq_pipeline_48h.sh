#!/usr/bin/env bash
# ----------------------------------------------------------------------
# Isolate Sequencing Pipeline v0.7 - 48h stage - (Assembly, QC, MLST, AMR)
# ----------------------------------------------------------------------
IFS=$'\n\t'
...
for idx in "${!SAMPLES[@]}"; do 
{
    ...
    sample_dir="/data/IsoSeq_results/${sample}/48h"
    ...
    ontime --to 48h -o "${sample_dir}/${sample}.fastq.gz" "${sample_dir}/${sample}_raw.fastq.gz"  # Use ontime to filter for only reads for this step
    ...
    log "✅ Sample $sample 48h analysis completed in ${total_time}s"
    exec 1>&3 2>&4
    exec 3>&- 4>&-
}; done
